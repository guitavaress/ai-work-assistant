...
- `src/work_assistant/db.py` — toda a persistência (SQLite em `~/.ai-work-assistant/assistant.db`). Schema: `tasks` (to-do diário, coluna `day`), `projects` (entregas com `goal`), `checkpoints` (relato + avaliação do modelo + `status`/`summary` para a web). Colunas novas entram via `_MIGRATIONS` (ALTER TABLE no `connect()`).
- `src/work_assistant/services.py` — lógica de negócio compartilhada (plan/checkpoint/standup/chat): monta contexto, chama o modelo e persiste. CLI e web SEMPRE passam por aqui — nada de montagem de prompt nos comandos ou na API.
