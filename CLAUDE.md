...
- `src/work_assistant/services.py` — lógica de negócio compartilhada (plan/checkpoint/standup/chat/review): monta contexto, chama o modelo e persiste. CLI e web SEMPRE passam por aqui — nada de montagem de prompt nos comandos ou na API. `review_metrics()` calcula as métricas de execução (atrasos, lead time, % não planejado, por tag/esforço) sem LLM; `run_review()` soma a avaliação do modelo.
